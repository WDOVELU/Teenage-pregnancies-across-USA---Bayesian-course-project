...
fix_rate <- rep(0,K*N)
integer_rate<- rep(0,K*N)
fraction_rate<-rep(0,K*N)
...
for (i in c(1:49))
{
  ...
  for (j in c(0:28))
  {
    ...
    integer_rate[i + K*j] <- floor(rate[j+1])
    fraction_rate[i + K*j] <- rate[j+1]-floor(rate[j+1])
...
mydata_integer <- data.frame(rep(states,29),years,integer_rate)
mydata_fraction <- data.frame(rep(states,29),years,fraction_rate)
...
{
  moran_subset <- subset(mydata_integer, years == i)
  mt <- moran.test(moran_subset$integer_rate,nb2listw(W.nb,style = "B"))
...
?ST.CARar
set.seed(1200)
formula1 <- integer_rate ~ integer_rate 
chain1 <- ST.CARar(formula = formula1, family = "poisson",
                   data = mydata_integer, W = W, burnin = 20000, n.sample = 220000,
                   thin = 100)
...
# (Intercept) 2.9144 2.9002 2.9276      1293.2        -0.1
# tau2        0.0114 0.0093 0.0142      1582.0         0.4
# rho.S       0.9877 0.9791 0.9933      2000.0        -0.4
# rho.T       0.9931 0.9774 0.9997      1845.3        -0.5
# 
# DIC =  7368.81       p.d =  227.9663       LMPL =  -3598.783
# 
...
#######################################################
mydata_integerNA<-mydata_integer
# texas in number 42, putting all years to NA
for (i in 0:28)
{
  mydata_integerNA$integer_rate[42+K*i]<-NA
}

formula1NA <- integer_rate ~ integer_rate
chain1NA_tex <- ST.CARar(formula = formula1NA, family = "poisson",
                     data = mydata_integerNA, W = W, burnin = 20000, n.sample = 220000,
                     thin = 100)
...
texas_measures <- subset(mydata_integer, mydata_integer$rep.states..29. == "texas")
texas<-data.frame(texas_measures,texasNA,high,low)
x11()
plot(c(c(1990:2018),c(1990:2018),c(1990:2018),c(1990:2018)),
     c(texas$integer_rate,texas$texasNA,texas$high,texas$low), 
...
MSE_texas <- MSE(texas$integer_rate, texasNA)
#  13.77823
...
for (i in 0:28)
{
  mydata_integerNA$integer_rate[24+K*i]<-NA
}

formula1NA <- integer_rate ~ integer_rate
chain1NA_miss <- ST.CARar(formula = formula1NA, family = "poisson",
                     data = mydata_integerNA, W = W, burnin = 20000, n.sample = 220000,
                     thin = 100)
...
missouri_measures <- subset(mydata_integer, mydata_integer$rep.states..29. == "missouri")
missouri<-data.frame(missouri_measures,low,missouriNA,high)
x11()
plot(c(c(1990:2018),c(1990:2018),c(1990:2018),c(1990:2018)),
     c(missouri$integer_rate,missouri$missouriNA,missouri$low,missouri$high), 
     pch = c(rep(19,29*2),rep(3,29*2)),
     col=c(rep("blue",29),rep("red",29*3)), main = 'Real vs sampled Missouri Birth rate values',
     xlab = "Year", ylab="Birth rate")

MSE_missouri <- MSE(missouri_measures$integer_rate, missouriNA)
#  2.118324
...
for (i in 0:28)
{
  mydata_integerNA$integer_rate[46+K*i]<-NA
}

formula1NA <- integer_rate ~ integer_rate
chain1NA_wash <- ST.CARar(formula = formula1NA, family = "poisson",
                     data = mydata_integerNA, W = W, burnin = 20000, n.sample = 220000,
                     thin = 100)
...
washington_measures <- subset(mydata_integer, mydata_integer$rep.states..29. == "washington")
washington<-data.frame(washington_measures,low,washingtonNA,high)
x11()
plot(c(c(1990:2018),c(1990:2018),c(1990:2018),c(1990:2018)),
     c(washington$integer_rate,washington$washingtonNA,washington$low,washington$high), 
     pch = c(rep(19,29*2),rep(3,29*2)),
     col=c(rep("blue",29),rep("red",29*3)), main = 'Real vs sampled Washington Birth rate values',
     xlab = "Year", ylab="Birth rate")


MSE_washington <- MSE(washington_measures$integer_rate, washingtonNA)
# 6.331506
...
#######################
mydata_integerNA3<-mydata_integer
# missouri in number 24, putting all years to NA
for (i in 26:28)
{
  mydata_integerNA3$integer_rate[24+K*i]<-NA
}

formula1NA <- integer_rate ~ integer_rate
chain1NA_miss3 <- ST.CARar(formula = formula1NA, family = "poisson",
                          data = mydata_integerNA3, W = W, burnin = 20000, n.sample = 220000,
                          thin = 100)
...
plot(c(c(1990:2018),c(2016:2018),c(2016:2018),c(2016:2018)),
     c(missouri_measures$integer_rate,missouri3$missouriNA,missouri3$low,missouri3$high), 
...
MSE_Missouri_3NA <- MSE(missouri_measures$integer_rate[27:29], missouriNA3)
# 0.10557